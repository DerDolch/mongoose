...
describe UsersHelper do
  ...
  describe "link_to_user" do
    ...
    it "should use given link text if :content_text is specified" do
      link_to_user(@user, :content_text => 'user_name (FirstName LastName)').should have_tag("a", 'user_name (FirstName LastName)')
    end
    it "should use the login as link text with no :content_method specified" do
      link_to_user(@user).should have_tag("a", 'user_name (FirstName LastName)')
    end
    it "should use the name as link text with :content_method => :name" do
      link_to_user(@user, :content_method => :first_name).should have_tag("a", 'user_name (FirstName LastName)')
      link_to_user(@user, :content_method => :last_name).should have_tag("a", 'user_name (FirstName LastName)')
    end
    ...
    it "should use the name as link title with :content_method => :name" do
      link_to_user(@user, :title_method => :first_name).should have_tag("a[title='FirstName']")
      link_to_user(@user, :title_method => :last_name).should have_tag("a[title='LastName']")
    end
  ...
  describe "link_to_current_user, When logged in" do
    ...
    it "should use given link text if :content_text is specified" do
      link_to_current_user(:content_text => 'user_name (FirstName LastName)').should have_tag("a", 'user_name (FirstName LastName)')
    end
    it "should use the login as link text with no :content_method specified" do
      link_to_current_user().should have_tag("a", 'user_name (FirstName LastName)')
    end
    it "should use the name as link text with :content_method => :first_name, :content_method => :last_name" do
      link_to_current_user(:content_method => :first_name).should have_tag("a", 'user_name (FirstName LastName)')
      link_to_current_user(:content_method => :last_name).should have_tag("a", 'user_name (FirstName LastName)')
    end
    it "should use the login as title with no :title_method specified" do
      link_to_current_user().should have_tag("a[title='user_name']")
    end
    it "should use the name as link title with :content_method => :name" do
      link_to_current_user(:title_method => :first_name).should have_tag("a[title='FirstName']")
      link_to_current_user(:title_method => :last_name).should have_tag("a[title='LastName']")
    end
